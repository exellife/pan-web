  ...
  <div>
    ...
    <section class="relative bg-steel-50 border-b border-steel-200 py-16 lg:py-24">
      ...
      <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <p class="text-accent-600 font-semibold text-sm uppercase tracking-widest mb-2">{{ $t('cookware.eyebrow') }}</p>
        <h1 class="text-4xl md:text-5xl font-display font-bold text-steel-900">{{ $t('cookware.title') }}</h1>
        <p class="mt-4 text-lg text-steel-600 max-w-2xl">
          {{ $t('cookware.subtitle', { count: products.length, collections: collections.length }) }}
        </p>
      </div>
    </section>
    ...
    <div class="lg:hidden sticky top-16 z-30 bg-white/95 backdrop-blur border-b border-steel-200 shadow-sm">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 py-3 space-y-3">
        ...
        >
          <option value="All">{{ $t('cookware.allCollections') }} ({{ products.length }})</option>
        ...
        <div class="lg:grid lg:grid-cols-[16rem_1fr] lg:gap-12">
          ...
          <aside class="hidden lg:block">
            <nav class="lg:sticky lg:top-20">
              <h2 class="text-xs font-semibold text-steel-400 uppercase tracking-widest mb-3">{{ $t('cookware.collectionsHeading') }}</h2>
              <ul class="space-y-0.5 max-h-[70vh] overflow-y-auto pr-2">
                <li>
                  ...
                  >
                    <span>{{ $t('cookware.allCollections') }}</span>
          ...
          <div>
            ...
            <p class="text-steel-400 text-sm mb-6">{{ $t('cookware.showing', { shown: filteredProducts.length, total: products.length }) }}</p>
            ...
            <p v-else class="text-steel-400 py-12">
              {{ $t('cookware.noMatch') }} <button class="text-accent-600 underline" @click="activeType = 'All'">{{ $t('cookware.resetType') }}</button>.
            </p>
          </div>
        </div>
      </div>
